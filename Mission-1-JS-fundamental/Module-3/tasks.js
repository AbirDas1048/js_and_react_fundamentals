...
let product = products?.filter((p) => p.inStock === true)?.sort((a, b) => a.price - b.price)?.map((p) => `${p.title},(${p.price})`);

console.log(product);

let totalPrice = products?.filter((p) => p.inStock === true)?.reduce((total, p) => total += p.price, 0);
